...
def invert(img_matrix):
    for i in range(len(img_matrix)):
        for n in range(len(img_matrix[i])):
            for m in range(len(img_matrix[i][n])):
                img_matrix[i][n][m] = 255 - img_matrix[i][n][m]
...
def grayscale(img_matrix):
    ...
    for i in range(len(img_matrix)):
        for n in range(len(img_matrix[i])):
            for m in range(1):
                num = int((img_matrix[i][n][m] + img_matrix[i][n][m+1] + img_matrix[i][n][m+2])/3)               
                img_matrix[i][n][m] = num
                img_matrix[i][n][m+1] = num
                img_matrix[i][n][m+2] = num
    #print("Not Yet Implemented")
    return img_matrix
    ...
    for i in range(len(m)):
        for n in range(len(m[i])):
            if (i == 0)or (n == 0):
                ...
                for j in range(3):
                    m[i][n][j] = 8*img_matrix[i][n][j] - img_matrix[i-1][n-1][j] - img_matrix[i][n-1][j] - img_matrix[i+1][n-1][j] - img_matrix[i-1][n][j] - img_matrix[i+1][n][j] - img_matrix[i-1][n+1][j] - img_matrix[i][n+1][j] - img_matrix[i+1][n+1][j]
                    if m[i][n][j] > 255:
                        m[i][n][j] = 255
                    if m[i][n][j] < 0:
                        m[i][n][j] = 0
